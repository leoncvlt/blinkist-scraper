...
def scraped_audio_exists(book_json):
  ...
  for chapter in enumerate(chapters):
    ...
    if (chapter_audio_exists):
      # add chapter_audio_path to existing_audio_files
  ...
  if not (os.path.exists(concat_audio)):
    if ((len(existing_audio_files) == chapter_count)): 
      ...
      print(f"[.] Audio for all {chapter_count} blinks already exists")
    else:
      if (len(existing_audio_files) > 0): 
        print(f"[.] Found audio files for {len(existing_audio_files)} out of {chapter_count} blinks")
      return False;
  else:
    print("[.] Concatenated audio already exists")
  return True;
